...
(* helper functions *)
  ...
  let binds_to_lists binds =
    ...
    (Symbol(arg), valu) | _ -> raise X_syntax_error) binds) 
  (* binds_to_lists - great done. *)
  let extract_syms pr = 
    list_and_map_pairs 
    (fun a -> match a with | (Symbol(x)) -> x 
    | _ -> raise X_syntax_error) pr;;
  (* extract_syms - good. *)
  let last lst = 
...
(* secondary tparse functions *)
  ...
  and tparse_quasiquote exps = 
    match exps with
    | Pair(Symbol("unquote"), Pair(sexp, Nil)) -> tparse sexp
    | Nil | Symbol(_) -> tparse (make_quote exps)
    | Pair(Pair(Symbol("unquote-splicing"),Pair(x, Nil)),y) -> Applic(Var("append"), [tparse x ;tparse_quasiquote y ] )
    | Pair(x,Pair(Symbol("unquote-splicing"),Pair(y, Nil))) -> Applic(Var("cons") , [tparse_quasiquote x ; tparse y])
    | Pair(x,y) -> Applic(Var("cons") , [tparse_quasiquote x; tparse_quasiquote y])
    | _ -> raise X_syntax_error
  (* tparse_quasiquote - quite good. *)
  ...
  and tparse_seq sexps = 
    let temp = (flattten_seq(list_and_map_pairs tparse sexps)) in 
